function [k, total, t_max, position] = Uneven(S, N, m, m_vm, B, u, u_agent, u_array, u_agent_array, S_r, n_r, type)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Uneven.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     %创建人：Willian Yu
...
for k = 1: 1: m
    ...
    n_agent = Na;
    %t2所有需要被转发的vm等待的总时间
    % 没有agent的vm随机分布给k个server
    t2 = (N - Na)/k * S/(N*B) * (N-Na);
    t3 = 0;
    M = N - Na;
    z = rand(1, 10);
    z = round(z * M/sum(z));
    t_max_u = t1/N + max(z)*S/(N*B);
    for i = 1: 1: n
        index = randi([1 N]);
        t3 = t3 + S/N/u_array(index);
        if (t1/N + max(z)*S/(N*B) + S/N/u_array(index) > t_max_u)
            t_max_u = t1/N + max(z)*S/(N*B) + S/N/u_array(index);
        end
    end
    t_max_u_agent = t1/N;
    for i = 1: 1: n_agent
    ...
    % 算出总时间
    t_reduce = max(z)*S_r*n_r/B + (N*S_r*n_r/B - S_r/B*Nb) + N*S_r/B*n_r_noagent + N*S_r*n_r/u;
    t_max(k, 2) = t_max(k, 2) + max(z)*S_r*n_r/B + N*S_r*n_r + N*S_r/u;
    if (isFind == 1)
